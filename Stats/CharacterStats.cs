...
public class CharacterStats : MonoBehaviour
{
    ...
    {
        Collider2D[] colliders = Physics2D.OverlapCircleAll(transform.position, 8); // ---> ShockStrike Range = 8.
    ...
    protected virtual void DecreaseHealthBy(int _damage)
    {
        ...
        if (_damage > 0)
        {
            if (igniteDamage == _damage)
            {
                fx.CreateIgnitePopUpText(_damage.ToString());
            }
            else if (shockDamage == _damage)
            {
                fx.CreateShockPopUpText(_damage.ToString());
            }
            else
